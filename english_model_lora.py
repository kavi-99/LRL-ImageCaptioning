...
Changes in this version:
  - Projection runs in fp16 natively (fixes NaN — was casting fp32→fp16 after MLP)
  - Feature stats printed after precompute and after loading from cache
  - Gradient norm logged per epoch to catch exploding gradients early
  - Loss NaN guard with diagnostics — raises immediately instead of running 10 epochs
  - Sample caption preview after each epoch (5 val images, pred vs reference)
  - Sample preview also runs before epoch 1 so you can see untrained baseline
  - VRAM logged at each major stage
"""
...
if os.path.exists(FEATURE_FILE):
    ...
    print(f"  Loaded {len(captions)} captions for {len(all_features)} images")
    ...
    captions            = []
    caption_to_img_idx  = []
    for i, s in enumerate(flickr):
        for j in range(5):
            captions.append(s[f"caption_{j}"])
            caption_to_img_idx.append(i)
    print(f"  Total captions: {len(captions)} from {len(images)} images")
...
# after loading mT5, instead of just freezing it:
...
# → trainable params: ~7M  (out of 580M total)
...
class ProjectionMLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return x
'''
# FIX: projection in fp16 so it matches mT5 — no cast needed anywhere
'''
...
class PrecomputedDataset(Dataset):
    def __init__(self, features, captions, caption_to_img_idx, tokenizer, max_length=64):
        self.features   = features
        self.captions   = captions
        self.caption_to_img_idx = caption_to_img_idx
    ...
    def __getitem__(self, idx):
        feature = self.features[self.caption_to_img_idx[idx]]  # (32, 768) float32 — cast to fp16 in train loop
...
full_dataset = PrecomputedDataset(all_features, captions, caption_to_img_idx, mt5_tokenizer)
...
def preview_captions(epoch_label="untrained"):
    ...
    for rank,cap_idx in enumerate(PREVIEW_INDICES):
        img_idx = caption_to_img_idx[cap_idx]
        feature = all_features[img_idx].unsqueeze(0).to(device)
        ref     = flickr[img_idx]["caption_0"]
        with torch.no_grad():
...
def train_epoch(loader, optimizer):
    ...
    for batch_i, (features, labels) in enumerate(tqdm(loader, desc="Train", leave=False)):
        ...
        optimizer.zero_grad()
        loss.backward()
        gnorm = torch.nn.utils.clip_grad_norm_(list(projection.parameters()) + list(mt5_model.parameters()), max_norm=1.0)
...
EPOCHS    = 10
optimizer = torch.optim.AdamW(list(projection.parameters()) + list(mt5_model.parameters()), lr=3e-4, weight_decay=1e-2)
...
ckpt = torch.load("outputs/best_checkpoint.pt", weights_only=True)
...
for start in tqdm(range(0, len(val_indices), EVAL_BATCH)):
    ...
    features  = torch.stack(
        [all_features[i] for i in img_idx]
    ).to(device)
    ...
    for k, cap_idx in enumerate(batch_idx):
        ...
        refs = [flickr[img_i][f"caption_{j}"] for j in range(5)]
        global_i             = start + k
